...
{
	public const TEXT_EMAIL_VERIFICATION_DEFAULT = 'Revisa tu Buzón de Email para recuperar su contraseña.';
	...
	public function registerUsersAction(
		...
		if ($request->isMethod('POST') && $this->isCsrfTokenValid('validateTokenSym', $submittedToken)) {
			...
			$userEmailCheck = $this->userRepository->findOneBy(['email' => $datosForm['email']]);
	...
	private function registerAlertsForInvalidEmailUser(
		?Usuario $userEmailCheck,
		array $dataForm,
		bool $onlyValidatePassword = false
	): string {
	...
	public function cuentaUsuarioControllerAction(
		Request $request,
		UserPasswordHasherInterface $userPasswordHasher
	): Response {
	...
	public function recoveryAccountUser(
		Request $request,
		string $alertMessage = self::TEXT_EMAIL_VERIFICATION_DEFAULT
	): Response {
		$submittedToken = $request->request->get('token');
		...
		if ($request->isMethod('POST') && $this->isCsrfTokenValid('validateTokenSym', $submittedToken)) {
			...
			$userEmailCheck = $this->userRepository->findOneBy(['email' => $datosForm[$emailForm]]);
			if ($userEmailCheck) {
				$uuid = Uuid::v4();
	...
	public function recoveryAccountUserValidateToken(
		Request $request,
		UserPasswordHasherInterface $userPasswordHasher
	): Response {
	...
	public function deleteAccountValidateTokenUserControllerAction(Request $request): Response
	{
		...
		if ($token) {
			...
			if ($userIdSession === $userIdToken && (bool) ($dateToken > new \DateTimeImmutable('now'))) {
				$local = $this->localRepository->findOneBy(['usuario' => $userIdSession]);
				if ($local->getLogo()) {
					$this->managePhoto->deletePhoto($local->getLogo());
				}
				$this->localRepository->remove($local, true);

				$menuData = $this->menuRepository->findBy(['userId' => $userIdSession]);
				...
				$productsData = $this->productRepository->findBy(['userId' => $userIdSession]);
				...
				$usuarioRecovery = $this->entityManager->getRepository(UsuarioRecovery::class)->findOneBy(
					['usuario' => $userIdSession]
				);
				if ($usuarioRecovery) {
					$this->entityManager->remove($usuarioRecovery);
				}

				$usuarioDeleteAccount = $this->entityManager->getRepository(UsuarioDeleteAccount::class)->findOneBy(
					['usuario' => $userIdSession]
				);
				$this->entityManager->remove($usuarioDeleteAccount);

				$user = $this->userRepository->findOneBy(['id' => $userIdSession]);
